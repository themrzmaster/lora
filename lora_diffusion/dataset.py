...
import glob
from .preprocess_files import face_mask_google_mediapipe, clipseg_mask_generator
...
class PivotalTuningDatasetCapation(Dataset):
    ...
    def __init__(
        ...
        # pre generate masks
        if self.use_clipseg_mask:
            self.clip_masks = []
            self.clip_masked_img = []
            for file in self.instance_images_path:
                image = Image.open(file)
                image = self.image_transforms(image)
                mask, seg_mask = clipseg_mask_generator(image, clipseg_caption)
                self.clip_masks.append(mask)
                self.clip_masked_img.append(seg_mask)
